...
fn get_logs_directory_parent_path() -> PathBuf {
    ...
    logs_directory_path.parent().expect("Failed to get logs directory parent! It might not be available.").to_path_buf()
}

fn get_latest_log_symlink_path() -> PathBuf {
    let logs_directory_parent_path = get_logs_directory_parent_path();
    let symlink_path_raw = format!("{}/latest.log", logs_directory_parent_path.to_str().unwrap());
...
pub fn validate_log_directory_setup() {
    ...
    if !logs_directory_path.clone().exists() {
        fs::create_dir_all(logs_directory_path.clone())
            .expect(&format!("Failed to create the logs directory! ({}) Logs won't be available.", logs_directory_path.clone().to_str().unwrap()));
    }
    ...
    let new_log_file_path = format!("{}/{}", logs_directory_path.to_str().unwrap(), new_log_filename);
...
pub fn set_logs_to_string_array(new_logs: Vec<String>) {
    let latest_log_symlink_path = get_latest_log_symlink_path();
    let mut latest_log_symlink = fs::File::create(latest_log_symlink_path).expect("Failed to open latest log file symlink!");

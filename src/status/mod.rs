...
impl StatusServer {
    ...
    pub async fn start_blocking(self: Arc<Self>) {
        let listener = TcpListener::bind(self.address).await.unwrap_or_else(|_| {
            panic!(
                "Failed to start the StatusServer on address {}",
                self.address
            )
        });
        ...
        loop {
            let (tcp, _) = listener
                .accept()
                .await
                .expect("Failed to accept a TCP connection!");
            ...
            tokio::task::spawn(async move {
                if let Err(error) = http1::Builder::new()
                    ...
                    .serve_connection(
                        io,
                        service_fn(|req| status_handler(Arc::clone(&self_clone), req)),
                    )
                    .await
                {
...
pub fn status_server() -> Arc<StatusServer> {
    STATUS_SERVER
        .get_or_init(|| Arc::new(StatusServer::new()))
        .clone()
}
